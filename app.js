...
const openSongPlayerNav = document.querySelector("#openSongPlayerNav");
...
const openSongPlayer = (songQuery) => {
  setActiveNav("songs");
  if (window.HelloLILIPlayer?.open) {
    window.HelloLILIPlayer.open({ query: songQuery });
  }
};
...
const syncNavFromHash = () => {
  ...
  if (window.HelloLILIPlayer?.close) {
    window.HelloLILIPlayer.close();
  }
...
const renderPlan = () => {
  ...
  songLink.dataset.songQuery = plan.song.title;
  songLink.setAttribute("aria-label", `播放 ${plan.song.title} 儿歌`);
...
openSongPlayerNav.addEventListener("click", () => {
...
songLink.addEventListener("click", () => {
  const plan = getPlan();
  window.location.hash = "songs";
  openSongPlayer(plan.song.title);
});
