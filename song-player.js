(() => {
  const songPlayerView = document.getElementById('songPlayerView');
  const closeSongPlayerButton = document.getElementById('closeSongPlayerButton');
  ...
  if (!songPlayerView || !songSelect || !videoPlayer || !subtitleOverlay || !subEn || !subZh || !playerContainer) {
    return;
  }
  ...
  const selectSongByQuery = (query) => {
    const normalizedQuery = normalizeSearchText(query);
    if (!normalizedQuery) return false;

    const options = Array.from(songSelect.options).filter(option => option.value);
    const matchedOption = options.find((option) => {
      const song = songsByPath.get(option.value);
      const optionText = normalizeSearchText(option.textContent);
      const songTitle = normalizeSearchText(song?.title);
      return optionText.includes(normalizedQuery) ||
        songTitle.includes(normalizedQuery) ||
        normalizedQuery.includes(songTitle);
    }) || options.find((option) => {
      const song = songsByPath.get(option.value);
      const looseOption = normalizeSearchText(option.textContent).replace(/\bthe\b/g, "").trim();
      const looseQuery = normalizedQuery.replace(/\bthe\b/g, "").trim();
      const looseTitle = normalizeSearchText(song?.title).replace(/\bthe\b/g, "").trim();
      return looseOption.includes(looseQuery) || looseQuery.includes(looseTitle);
    });

    if (!matchedOption) return false;

    songSelect.value = matchedOption.value;
    songSelect.dispatchEvent(new Event('change', { bubbles: true }));
    return true;
  };

  const openPlayer = ({ query } = {}) => {
    songPlayerView.hidden = false;
    document.body.classList.add('song-player-open');
    if (query) {
      selectSongByQuery(query);
    }
  };

  const closePlayer = () => {
    videoPlayer.pause();
    songPlayerView.hidden = true;
    document.body.classList.remove('song-player-open');
  };

  closeSongPlayerButton?.addEventListener('click', () => {
    closePlayer();
  ...
  window.HelloLILIPlayer = {
    open: openPlayer,
    close: closePlayer,
    select: selectSongByQuery,
    isLooping: () => videoPlayer.loop === true,
  };
})();
